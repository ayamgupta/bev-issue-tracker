...
export const COMMUNITY_ISSUES: CommunityIssue[] = [
  {
    ...
    fix: 'A full power cycle (shut down completely, wait 20-30s for the shutdown click/animation, restart) resets the suspension controller and is the most reliable immediate fix. Mahindra\'s FSWU017/019/021 OTA updates progressively improved damping for many owners. For persistent cases, service centres reflash the Front/Mid/Rear Zonal ECUs, recalibrate or replace the suspension (G-)sensor, and Mahindra has run a suspension-sensor TSB/recall for cars built before June 2025. Custom Drive Mode (from software v5) lets you pin the firmer Race-mode suspension setting to avoid the softer modes that seem to trigger the fault.',
  },
  {
    ...
    fix: 'Fixed for many owners once the amplifier ECU is updated to firmware HD476 or later — this can arrive via the FSWU021 OTA update, or can be requested/manually flashed at a service centre via the MIDA diagnostic tool if the OTA hasn\'t reached your car. DIY workaround reported by some: set all EQ sliders to minimum, tap Reset, then re-raise bass/sub toward max before other bands.',
  },
  {
    ...
    fix: 'An immediate ignition off/on (or a 5-10 minute full shutdown) releases the lock as a stopgap. The permanent fix is replacement of the steering pigtail connector, which Mahindra has been handling as an unofficial/reactive recall — owners report being contacted by service centres rather than receiving a public recall notice, so proactively ask your SC to check the pigtail if you experience this.',
  },
  {
    ...
    fix: 'Ask the service centre to isolate the actual leak rather than just topping up coolant — top-ups alone tend to recur within weeks. Mahindra has issued a recall covering the degassing tank cap; if that doesn\'t resolve it, the water pump may need replacement (has taken up to 8 days at some service centres due to parts availability).',
  },
  {
    ...
    fix: 'For throttled/dropped sessions: lock the car, move it slightly, and reconnect the charger — this resets the negotiated rate for many owners. If the same charger works fine on another EV, or the issue follows the car across multiple chargers, ask the service centre to check the OCDC module (may require a warranty replacement, and parts can take weeks to arrive).',
  },
  {
    ...
    fix: 'Check your MCB rating and earthing first — several cases traced to weak home earthing or an undersized circuit; a voltage stabilizer resolved it for some. If it stalls at 99%, this is often normal BMS cell-balancing behaviour that resolves after a few charge cycles, or try a slower charger (3.3kW reached 100% for some owners when 7.2kW didn\'t). Persistent AC-only failures with no wiring cause may indicate an OCDC fault — see DC fast-charging entry above.',
  },
  {
    ...
    fix: 'Turning Auto Hold off resolves it immediately for the current drive. If it recurs, get the brake/Auto Hold system inspected at a service centre — some owners now avoid Auto Hold in stop-start traffic as a precaution.',
  },
  {
    ...
    fix: 'A deep-sleep restart (lock and leave the car for 15-30 minutes) clears it temporarily for some owners. The lasting fix is replacement of the gear shifter/Park switch assembly under warranty — several owners report Mahindra covering the full shifter-set replacement at no cost once diagnosed.',
  },
  {
    ...
    fix: 'If the warning is intermittent, driving normally recharges the LV battery from the HV pack and no action is needed per Mahindra technicians — but get it checked if it recurs often. If the car becomes unresponsive (no unlock/no start), the LV battery usually needs replacement under warranty; Mahindra reportedly switched 12V battery brand after a cluster of early failures.',
  },
  {
    ...
    fix: 'Try a deep-sleep reset first: lock the car and walk away with the key for 15-30 minutes for a full shutdown/reboot. If that doesn\'t clear it, check the 12V battery terminal is tight. Persistent cases usually need a flatbed tow to a service centre — reported root causes include a faulty OCDC module and a disconnected HV plug that SC technicians reseated.',
  },
  {
    ...
    fix: 'Escalating to Mahindra\'s central customer care (rather than the local SC alone) has gotten full 4-tyre replacements approved in some disputed cases. Many owners have proactively switched to aftermarket Michelin, Continental, Bridgestone or MRF tyres. Mahindra has reportedly been discussing a Goodyear-to-MRF Markus tyre swap program — worth asking your SC if you\'re affected.',
  },
  {
    ...
    fix: 'A cold reboot (full shutdown, wait for the relay-disconnect click, ~2 minutes) temporarily restores it for most owners. If it\'s a genuine radar/camera fault (confirm the car actually has that sensor for its pack level), the part gets replaced and recalibrated at a service centre — parts lead time has run up to ~10 days in reported cases.',
  },
  {
    ...
    fix: 'No confirmed permanent fix; owners describe it as a known industry-wide "phantom braking" limitation of camera/radar AEB systems. Some set the AEBS warning threshold to "early" to get 2-3 extra seconds of warning before auto-braking fires, or disable specific ADAS features in dense city traffic.',
  },
  {
    ...
    fix: 'Usually clears itself after a restart or two, though it can recur over following months. In one confirmed case a faulty LVDR cable was the root cause and replacing it fully resolved recurring screen restarts — worth asking your SC to check that cable if the issue is frequent rather than a one-off.',
  },
  {
    ...
    fix: 'Repair means wiring/pipe replacement at a service centre (₹2,500-₹35,000 depending on scope) and is not reliably covered under warranty since it\'s physical/pest damage — some owners claimed it via insurance instead. No repellent (sprays, tobacco strands) has proven consistently effective; several owners consider the exposed wiring layout a design flaw worth raising with Mahindra directly.',
  },
  {
    ...
    fix: 'A manual seat-calibration procedure helps some cars: hold the seat-base switch backward together with the memory (M) button for about 20 seconds to relearn the position. Service centres can also relearn it with a diagnostic laptop. For persistent cases hardware (seat motor) replacement under warranty has been needed.',
  },
  {
    ...
    fix: 'Toggling the proximity setting off/on and power-cycling the car fixes it for some. Persistent cases have needed a service-centre reflash of the HPCC and Front Zonal ECUs. The self-unlocking-while-charging behaviour has no confirmed fix yet — worth flagging to your SC given the theft-risk implications.',
  },
  {
    ...
    fix: 'For app-only glitches: log out and back in, or uninstall/reinstall the app. Server-side outages resolve on their own within a day or two — no owner-side fix exists for those. Persistent in-car connectivity loss has occasionally been resolved by a service centre disconnecting/reconnecting the LV battery to reset the telematics module.',
  },
  {
    ...
    fix: 'No consistent owner-side fix; some report it intermittently starting to work again with a 5-10s delay. One service centre told an owner this specific reset-on-power-off behaviour would not be addressed even in the next software update — worth confirming current status with your SC before assuming it\'s fixable.',
  },
  {
  ...
  },
  {
    ...
    fix: 'A brake-cleaner spray on the caliper/disc at the service centre gives temporary relief for most owners, but it tends to recur within weeks since the underlying regen-brake-underuse cause isn\'t fixed by cleaning. If it doesn\'t improve, ask the SC to check for a warped disc or misaligned caliper rather than accepting a repeat clean.',
  },
  {
    ...
    fix: 'Switch the audio source away from AA/CarPlay and back, or reconnect manually — usually restores it without a restart. Some owners traced part of this to a known Android Auto app-side bug (fixed by updating the Android Auto app itself, separate from the car\'s software); using a wired connection instead of wireless sidesteps it entirely for CarPlay pixelating.',
  },
  {
    ...
    fix: 'If stuck above ~90% for over an hour, it has very often actually finished — the on-screen percentage is just stale. Lock the car and leave the key away for 20-30 minutes to force a full clean shutdown/reboot, which usually registers the completed update. If it still shows as interrupted afterward, a service-centre visit is needed to re-flash.',
  },
  {
  ...
  },
  {
    ...
    fix: 'A loose connector is a common root cause for cooling dropouts — SC reseating or replacing it fixes many cases; a full Climate Control Module replacement has been needed when reflashing doesn\'t help. For persistent odor, replacing the cabin filter gives partial relief; evaporator cleaning (via the frunk access pipe) has helped when the SC agrees to do it, though some SCs decline saying it isn\'t needed. A recall exists for broken/faulty AC vents on some VINs — ask your SC directly if unsure it applies to your car.',
  },
  {
  ...
  },
  {
    ...
    fix: 'For a stuck handle, holding the small square icon/sensor near the handle for about 20 seconds has fixed it for some owners. For a door that releases but won\'t open, a service-centre DCM firmware reflash has resolved it; in the worst cases the entire door lock/handle unit needs replacement under warranty.',
  },
  {
    ...
    fix: 'Manually adjusting the HUD position/height in settings improves it for some. Service centres have offered a HUD glass replacement, but owners note this is a bigger electronics job with no guaranteed improvement — weigh that against how much it bothers you before requesting it.',
  },
  {
    ...
    fix: 'Stock blades wear or were poor quality for several owners, who switched to aftermarket Bosch or other rigid-frame blades for smoother, quieter operation. If the noise is a software issue rather than the blades, a wiper-calibration pass during an SC software reflash session has resolved it in at least one case.',
  },
  {
    ...
    fix: 'No confirmed owner-side fix. If this happens to you, report it to Mahindra immediately and request the vehicle\'s driving logs be pulled — this is a safety-critical fault that warrants formal escalation rather than a workaround.',
  },
  {
